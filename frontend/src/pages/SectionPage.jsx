...
const toCard = (m) => ({
  title: m.title,
  poster: m.poster_url,
  genre: m.category?.name || "Movie",
  rating: m.category?.name || "Now",
  date: m.release_date,
});
...
export default function SectionPage({ sections = [] }) {
  ...
  const [movieData, setMovieData] = useState({ nowShowing: [], comingSoon: [] });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setMovieData({
          nowShowing: data.filter((m) => new Date(m.release_date) <= today).map(toCard),
          comingSoon: data.filter((m) => new Date(m.release_date) > today).map(toCard),
        });
      } catch {
  ...
  const renderSection = (name) => {
    switch (name) {
      ...
      case "coming-soon": {
        const fixedTab =
          name === "coming-soon" ? "coming-soon" : "now-showing";
        const showTabs = name === "showtimes";
        const currentTab = showTabs ? activeTab : fixedTab;
        const movieList =
          currentTab === "now-showing" ? movieData.nowShowing : movieData.comingSoon;

        return (
          <section className="home-section" id="showtimes">
            <div className="home-section-head">
              <h2 className="home-section-title">Showtimes</h2>
              {showTabs && (
    ...
    <div className="home-page">
      ...
      <div className="home-page-banner">
        <h1 className="home-page-banner-title">
          {sections.find((s) => s !== "slider") || sections[0]}
        </h1>
      </div>

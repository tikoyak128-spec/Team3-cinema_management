...
export default function GalleryList() {
  ...
  useEffect(() => {
    let cancelled = false;
    api.get("/gallery-images").then(({ data }) => { if (!cancelled) setItems(Array.isArray(data) ? data : []); })
      .catch((err) => { if (!cancelled) setError(err?.response?.data?.message || t("adminGalleryList.failedLoad")); })
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setError(err?.response?.data?.message || t("adminGalleryList.failedDelete"));
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 text-[var(--app-ink)] [&_*]:box-border">
      ...
      <div className="flex items-center gap-2.5 bg-[var(--app-panel)] border border-[var(--app-edge)] rounded-xl py-2.5 px-3.5 text-[var(--app-mute)] w-full sm:max-w-xs">
        ...
        <input className="bg-transparent border-none outline-none text-[var(--app-ink)] font-inherit text-[14px] w-full" placeholder={t("adminGalleryList.searchPlaceholder")} value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
      ...
      <div className="bg-[var(--app-panel)] border border-[var(--app-edge)] rounded-2xl overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-10 px-5 text-center text-[var(--app-mute)]">
            <p>{t("adminGalleryList.loading")}</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-[60px] px-5 text-center text-[var(--app-mute)]">
            <div className="text-[44px] mb-3"><Images size={32} /></div>
            <p>{t("adminGalleryList.noData")}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 p-4 sm:p-5">
            {filtered.map((g) => (
              <div key={g.id} className="bg-[var(--app-panel2)] border border-[var(--app-edge2)] rounded-2xl overflow-hidden transition-colors hover:border-brand/40">
                <div className="relative h-[190px] overflow-hidden bg-[var(--app-fill)]">
                  {g.image ? (
                    <img src={g.image} alt={g.alt || g.title} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-[var(--app-mute)]"><Images size={40} /></div>
                  )}
                  {!g.is_active && (
                ...
                <div className="p-4">
                  <div className="font-bold text-[var(--app-ink)] leading-snug truncate">{g.title}</div>
                  <div className="text-[12px] text-[var(--app-mute)] mt-0.5 mb-3 truncate">{g.subtitle || "—"}</div>
                  <div className="pt-3 border-t border-[var(--app-edge)] flex items-center justify-between gap-2">
                    <span className="text-[12px] text-[var(--app-mute)]">#{String(g.id).padStart(3, "0")}</span>
                    {deleteButtons(g)}
                  </div>
                </div>
              </div>

...
export default function CategoriesList() {
  ...
  useEffect(() => {
    let cancelled = false;
    api.get("/categories").then(({ data }) => { if (!cancelled) setCategories(data); })
      .catch((err) => { if (!cancelled) setError(err?.response?.data?.message || t("adminCategoryList.failedLoad")); })
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setError(err?.response?.data?.message || t("adminCategoryList.failedDelete"));
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 text-[var(--app-ink)] [&_*]:box-border">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-[22px] sm:text-[26px] font-extrabold tracking-wide">{t("admin.categories")}</h1>
          <p className="text-[14px] text-[var(--app-mute)] mt-1">{t("adminCategoryList.subtitle")}</p>
        </div>
        <button className="inline-flex items-center justify-center gap-2 border-none cursor-pointer font-inherit py-[11px] px-5 text-[14px] font-bold rounded-xl transition-all duration-200 bg-[#e50914] text-white shadow-[0_4px_14px_rgba(229,9,20,0.35)] hover:bg-[#f40612] hover:-translate-y-px w-full sm:w-auto" onClick={() => navigate("/admin/categories/create")}><Plus size={16} /> {t("admin.addCategory")}</button>
      </div>
      ...
      <div className="flex items-center gap-2.5 bg-[var(--app-panel)] border border-[var(--app-edge)] rounded-xl py-2.5 px-3.5 text-[var(--app-mute)] w-full sm:max-w-xs">
        ...
        <input className="bg-transparent border-none outline-none text-[var(--app-ink)] font-inherit text-[14px] w-full" placeholder={t("adminCategoryList.searchPlaceholder")} value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
      ...
      <div className="bg-[var(--app-panel)] border border-[var(--app-edge)] rounded-2xl overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-10 px-5 text-center text-[var(--app-mute)]">
            <p>{t("adminCategoryList.loading")}</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-[60px] px-5 text-center text-[var(--app-mute)]">
            <div className="text-[44px] mb-3"><Tag size={32} /></div>
            <p>{t("adminCategoryList.noData")}</p>
          </div>
        ) : (
          <>
              ...
              {filtered.map((c) => (
                <div key={c.id} className="bg-[var(--app-panel2)] border border-[var(--app-edge2)] rounded-2xl p-4">
                  <div className="flex items-start gap-3">
                    <div className="w-[38px] h-[38px] rounded-full bg-[rgba(229,9,20,0.12)] flex items-center justify-center text-[16px] shrink-0"><Tag size={18} /></div>
                    <div className="min-w-0 flex-1">
                      <div className="font-bold text-[var(--app-ink)] leading-snug">{c.name}</div>
                      <div className="text-[12px] text-[var(--app-mute)] mt-0.5">CAT-{String(c.id).padStart(3, "0")}</div>
                    </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-[var(--app-edge)] flex flex-wrap items-center gap-x-4 gap-y-2 text-[13px]">
                    <span className="min-w-0 flex-1 text-[var(--app-mute)]">{c.description || "N/A"}</span>
                    <span className="inline-flex items-center gap-1.5 bg-[var(--app-fill)] border border-[var(--app-edge2)] text-[var(--app-ink2)] py-1 px-2.5 rounded-[8px] text-[12px] font-semibold">{t("adminCategoryList.moviesCount", { count: c.movies_count ?? 0 })}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full border-collapse text-[14px] [&>thead_th]:text-left [&>thead_th]:py-[14px] [&>thead_th]:px-[18px] [&>thead_th]:text-[var(--app-mute)] [&>thead_th]:text-[12px] [&>thead_th]:font-bold [&>thead_th]:uppercase [&>thead_th]:tracking-widest [&>thead_th]:border-b [&>thead_th]:border-[var(--app-edge)] [&>thead_th]:bg-[var(--app-fill)] [&>thead_th]:whitespace-nowrap [&>th]:sticky [&>th]:top-[70px] [&>th]:z-5 [&>th]:bg-[var(--app-panel)] [&>tbody_td]:py-[14px] [&>tbody_td]:px-[18px] [&>tbody_td]:border-b [&>tbody_td]:border-[var(--app-edge)] [&>tbody_td]:text-[var(--app-ink2)] [&>tbody_td]:align-middle [&>tbody>tr]:transition-colors [&>tbody>tr]:duration-150 [&>tbody>tr:hover]:bg-[var(--app-fill)] [&>tbody>tr:last-child>td]:border-b-0">
                <thead><tr><th>{t("adminCategoryList.category")}</th><th>{t("adminCategoryList.description")}</th><th>{t("adminCategoryList.movies")}</th><th>{t("common.actions")}</th></tr></thead>
                <tbody>
                  {filtered.map((c) => (
                    <tr key={c.id}>
                      <td>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-[12px] text-[var(--app-mute)]">CAT-{String(c.id).padStart(3, "0")}</div>
                          </div>
                        </div>
                      </td>
                      <td className="text-[12px] text-[var(--app-mute)]">{c.description || "N/A"}</td>
                      <td><span className="inline-flex items-center gap-1.5 py-[5px] px-3 text-[12px] font-bold rounded-[20px] whitespace-nowrap bg-[var(--app-fill)] text-[var(--app-mute)] border border-[var(--app-edge2)]">{t("adminCategoryList.moviesCount", { count: c.movies_count ?? 0 })}</span></td>

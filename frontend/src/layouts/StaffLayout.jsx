...
const staffNavSections = [
  {
    ...
    items: [
      {
        name: "Dashboard",
        nameKey: "staff.dashboard",
        icon: ChartColumn,
        path: "/staff/dashboard",
      },
      {
        name: "Profile",
        nameKey: "staff.profile",
        icon: UserRound,
        path: "/staff/profile",
      },
    ],
  ...
  {
    label: "Ticketing",
    labelKey: "staff.ticketing",
    items: [
      {
        name: "Sell Ticket",
        nameKey: "staff.sellTicket",
        icon: ShoppingCart,
        path: "/staff/sell",
      },
      {
        name: "Search Ticket",
        nameKey: "staff.searchTicket",
        icon: Search,
        path: "/staff/search",
      },
      {
        name: "Check-In",
        nameKey: "staff.checkIn",
        icon: CircleCheck,
        path: "/staff/checkin",
      },
    ],
  ...
  {
    label: "Management",
    labelKey: "staff.management",
    items: [
      {
        name: "Bookings",
        nameKey: "staff.bookings",
        icon: CalendarDays,
        path: "/staff/bookings",
      },
      {
        name: "Tickets",
        nameKey: "staff.tickets",
        icon: Ticket,
        path: "/staff/tickets",
      },
    ],
...
export default function StaffLayout({ children }) {
  ...
  useEffect(() => {
    staffNavSections.forEach((section) => {
        ...
        <div className="relative border-t border-[rgba(0,0,0,0.1)] p-3.5 dark:border-dark-border">
          ...
          >
            ...
            {!compact && (
              <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
                <span className="truncate text-[13px] font-semibold text-[#1a1a1a] dark:text-[#f5f5f5]">{user?.name || t("staff.staffMember")}</span>
                <span className="truncate text-[11px] text-muted">{user?.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : t("staff.ticketOfficer")}</span>
              </div>
            )}
            {!compact && (
          ...
          {!compact && profileOpen && (
            <div className="absolute bottom-full left-3.5 right-3.5 z-10 mb-2 flex flex-col gap-0.5 rounded-xl border border-[var(--app-edge2)] bg-[var(--app-panel2)] p-1.5 shadow-[0_-8px_24px_rgba(0,0,0,0.5)]">
              <button className="flex cursor-pointer items-center gap-2.5 rounded-lg px-3 py-2.5 text-left text-[13px] font-semibold text-[var(--app-ink2)] transition-colors duration-150 hover:bg-white/5 hover:text-white" onClick={() => navigate("/staff/profile")}>
                <CircleUser size={16} /> {t("staff.profile")}
              </button>
      ...
      <div className="flex h-full min-h-0 w-full flex-col overflow-hidden">
        ...
        <header className="z-[50] flex h-[70px] shrink-0 items-center justify-between border-b px-7 backdrop-blur-[12px] bg-[rgba(255,255,255,0.85)] dark:bg-[var(--app-header)] border-[rgba(0,0,0,0.1)] dark:border-dark-border transition-colors duration-300">
          <div className="flex items-center gap-2">
            <button className={btnBase} onClick={toggleSidebar} title={t(isMobile ? "staff.openSidebar" : compact ? "staff.expandSidebar" : "staff.collapseSidebar")}>
              <PanelLeft size={18} />
            </button>
            <div className="flex items-center gap-2 text-[14px]">
              ...
              <span className="font-semibold text-[#1a1a1a] dark:text-[#f5f5f5]">
                {(() => {
                  const active = staffNavSections
                    .flatMap((s) => s.items)
                    .flatMap((item) => [item, ...(item.children || [])])
                    .find((i) => i.path === activePath);
                  return t(active?.nameKey || active?.name || "staff.dashboard");
                })()}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2.5">
            ...
            <button className={btnBase} onClick={toggleTheme} title={t("staff.toggleTheme")}>
              {isDarkMode ? <Sun size={18} /> : <Moon size={18} />}
            </button>
            <div onClick={() => handleNav("/staff/profile")} className="ml-1.5 flex cursor-pointer items-center gap-2.5 rounded-[10px] border px-3 py-1.5 transition-colors duration-200 bg-[rgba(0,0,0,0.03)] dark:bg-[rgba(255,255,255,0.03)] border-[rgba(0,0,0,0.1)] dark:border-dark-border hover:bg-black/5 dark:hover:bg-white/5" title={t("staff.profile")}>
              ...
              <span className="max-md:hidden text-[13px] font-semibold">{user?.name || t("staff.staffMember")}</span>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 min-h-0 overflow-y-auto p-7">
          {children || (
            <div className="flex min-h-[450px] flex-col items-center justify-center gap-3 rounded-2xl border border-dashed p-10 text-center bg-white border-[rgba(0,0,0,0.1)] dark:bg-dark-card dark:border-dark-border">
              <div className="mb-1 text-brand"><Shield size={48} /></div>
              <h2 className="text-[22px] font-semibold text-[#1a1a1a] dark:text-[#f5f5f5]">{t("staff.staffDashboard")}</h2>
